...
1.  **Screening (Immunoassay):** A rapid and automated test that detects the presence of drug metabolites in the urine. A positive result here is considered a "presumptive positive."
2.  **Confirmation (GC-MS):** If the screening test is positive, the *same urine sample* must then be subjected to a confirmatory test using GC-MS to eliminate the possibility of a false positive and provide a legally defensible result.
...
1.  **Mandatory Drug Testing:** Required for:
...
2.  **Random Drug Testing:** Can be conducted for:
